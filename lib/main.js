...
class QueryFeature extends SqlOpsFeature {
    ...
    registerProvider(options) {
        ...
        let setQueryExecutionOptions = (ownerUri, options) => {
            let params = {
                ownerUri: ownerUri,
                options: options
            };
...
class ProfilerFeature extends SqlOpsFeature {
    ...
    registerProvider(options) {
        const client = this._client;
        let createSession = (ownerUri, sessionName, template) => {
            let params = {
                ownerUri,
                sessionName,
                template
            };
            return client.sendRequest(protocol.CreateXEventSessionRequest.type, params).then(r => true, e => {
                client.logFailedRequest(protocol.CreateXEventSessionRequest.type, e);
                return Promise.reject(e);
            });
        };
        let startSession = (ownerUri, sessionName) => {
            let params = {
                ownerUri,
                sessionName
            };
        ...
        let pauseSession = (ownerUri) => {
            let params = {
                ownerUri
            };
            return client.sendRequest(protocol.PauseProfilingRequest.type, params).then(r => true, e => {
                client.logFailedRequest(protocol.PauseProfilingRequest.type, e);
                return Promise.reject(e);
            });
        };
        let getXEventSessions = (ownerUri) => {
            let params = {
                ownerUri
            };
            return client.sendRequest(protocol.GetXEventSessionsRequest.type, params).then(r => r.sessions, e => {
        ...
        let disconnectSession = (ownerUri) => {
            let params = {
                ownerUri: ownerUri
            };
            return client.sendRequest(protocol.DisconnectSessionRequest.type, params).then(r => true, e => {
        ...
        let registerOnSessionStopped = (handler) => {
            client.onNotification(protocol.ProfilerSessionStoppedNotification.type, (params) => {
                handler({
        ...
        let registerOnProfilerSessionCreated = (handler) => {
            client.onNotification(protocol.ProfilerSessionCreatedNotification.type, (params) => {
                handler({
                    ownerUri: params.ownerUri,
...
exports.ProfilerFeature = ProfilerFeature;
/**
 *
 */
